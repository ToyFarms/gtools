...
class TankPacket(Serializable):
    ...
    @classmethod
    def deserialize(cls, data: bytes) -> "TankPacket":
        ...
        if values[-1] != len(extended_data):
            raise RuntimeError(
                f"extended data size does not match (in the packet: {values[-1]}, actual: {len(extended_data)})"
            )
...
if __name__ == "__main__":
    ...
    for sample in samples:
        pkt = NetPacket.deserialize(binascii.unhexlify(sample))
        print(pkt.serialize())
