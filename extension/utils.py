...
class UtilityExtension(Extension):
    ...
    @dispatch(s.command_toggle("/ping", s.auto))
    def _ping(self, _event: PendingPacket) -> PendingPacket | None:
        self.console_log(f"server={self.state.me.server_ping} client={self.state.me.client_ping}")

        return self.cancel()

    @dispatch(s.command_toggle("/exit", s.auto))
    def _exit(self, _event: PendingPacket) -> PendingPacket | None:
        self.to_main_menu()
    ...
    def _block_extra_quit_to_exit(self, _event: PendingPacket) -> PendingPacket | None:
        if self.should_block:
            print("BLOCKED EXTRA QUIT_TO_EXIT")
            self.should_block = False
    ...
    @dispatch(s.command("/warp", s.auto))
    def _warp(self, event: PendingPacket) -> PendingPacket | None:
        if not self.intercept_warp:
            return self.pass_to_next()

        self.warp_target = s.parse_command(event)
    ...
    def _exited(self, _event: PendingPacket) -> PendingPacket | None:
        if not self.intercept_warp or not self.warp_target:
            return
    ...
    @dispatch(s.command_toggle("/fd", s.auto))
    def _fast_drop_toggle(self, _event: PendingPacket) -> PendingPacket | None:
    ...
    def _fast_drop_request(self, event: PendingPacket) -> PendingPacket | None:
    ...
    def _gazette_dialog(self, _event: PendingPacket) -> PendingPacket | None:
    ...
    @dispatch(s.command("/find", s.auto))
    def _find(self, event: PendingPacket) -> PendingPacket | None:
        ...
        t = 0
        for tile in found:
            for tile_inbetween in bresenham(
                int(self.state.me.pos.x // 32),
                int(self.state.me.pos.y // 32),
                tile.pos.x,
                tile.pos.y,
            ):
    ...
    @dispatch(s.command("/setdrop", s.auto))
    def _setdrop(self, event: PendingPacket) -> PendingPacket | None:
        ...
        self.fast_drop_amt = int(amt)
        self.console_log(f"fast drop amount set to {self.fast_drop_amt}")
