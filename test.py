...
WORLD_DIR = "/mnt/c/Users/andra/.gtworlds"
    ...
    for idx, tile in enumerate(w.tiles):
        for id in (tile.fg_id, tile.bg_id):
            try:
                ...
                tex = manager.get(f"/mnt/c/Users/andra/AppData/Local/Growtopia/game/{i.texture_file.decode()}", i.tex_coord_x * 32, i.tex_coord_y * 32, 32, 32)
                img.paste(tex, (tile.x * 32, tile.y * 32))
                if w.garbage_start != -1 and idx >= w.garbage_start:
                    draw = ImageDraw.Draw(img)
                    draw.rectangle((tile.x * 32, tile.y * 32, tile.x * 32 + 32, tile.y * 32 + 32), outline=(255, 0, 0, 128), width=2)
            except:
                draw = ImageDraw.Draw(img)
                draw.rectangle((tile.x * 32, tile.y * 32, tile.x * 32 + 32, tile.y * 32 + 32), outline=(0, 255, 0, 128), width=2)
        if idx == w.garbage_start:
            draw.rectangle((tile.x * 32, tile.y * 32, tile.x * 32 + 32, tile.y * 32 + 32), outline=(100, 255, 100, 128), width=32)
